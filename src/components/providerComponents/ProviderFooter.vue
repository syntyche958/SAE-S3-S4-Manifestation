<template>
  ...
  <Dialog v-model:visible="visible" :header="$t('message.contactProvider')" :style="{ width: '30rem' }" :modal="true"
    ...
    <div class="flex items-center gap-4 mb-8">
      ...
      <Select v-model="selectedProvider" :options="providers" optionLabel="name" placeholder="Select the provider"
    ...
    <div class="flex items-center gap-4 mb-8">
      ...
      <Select disabled v-model="selectedActivity" :options="activities" optionLabel="name"
        placeholder="Select the activity" class="w-full md:w-56" />
    </div>
    ...
    <div class="flex justify-end gap-2">
      ...
      <Button type="button" :label="$t('message.send')"
        ...
        @click="sendRequest()" :disabled="btnDisabled"></Button>
    </div>
...
const btnDisabled = computed(() => message.value != '' && visitorMail.value != '' && selectedProvider.value ? false : true)
const message = ref('')
